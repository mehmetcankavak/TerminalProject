            ...
            <div className="nt-topbar-right">
                ...
                {marginUsed > 0 && (
                    <span title="Margin Used — açık pozisyonlara ayrılmış teminat" style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4 }}>
                        ...
                        <span style={{ color: "var(--ct-warning, #f5a623)" }}>${fmt(marginUsed)}</span>
                    </span>
                )}
                <span title="Unrealized PnL — açık pozisyonların şu anki kâr/zararı" style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4 }}>
                    ...
                    <span style={{ color: workspaceTextColor(unrealizedTotal >= 0 ? 'var(--accent)' : 'var(--danger)'), fontWeight: 700 }}>{fmtPnl(unrealizedTotal)}</span>
                </span>
                <span title="Realized PnL (bugün)" style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4 }}>
                    ...
                    <span style={{ color: workspaceTextColor(realizedToday >= 0 ? 'var(--accent)' : 'var(--danger)') }}>{fmtPnl(realizedToday)}</span>
                </span>
                {connect.tradingMode === 'LIVE' ? (
                    connect.hlTestnet ? (
                        <span style={{ background: '#f5a62322', color: "var(--ct-warning, #f5a623)", border: '1px solid #f5a62366', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: 1 }} title="HL TESTNET — gerçek para değil, mock USDC">
                            ⚠ TESTNET · HL {connect.hlWallet && <span style={{ fontWeight: 400, opacity: 0.8 }}>{connect.hlWallet}</span>}
                        </span>
                    ) : (
                        <span style={{ background: '#00d99222', color: "var(--ct-positive, #00d992)", border: '1px solid #00d99244', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: 1 }} title="HL MAINNET — gerçek para">
                            ● MAINNET · HL {connect.hlWallet && <span style={{ fontWeight: 400, opacity: 0.8 }}>{connect.hlWallet}</span>}
                        </span>
                    )
                ) : connect.tradingMode === 'LIVE_BINANCE' ? (
                    connect.bnbTestnet ? (
                        <span style={{ background: '#f5a62322', color: "var(--ct-warning, #f5a623)", border: '1px solid #f5a62366', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: 1 }} title="Binance TESTNET">⚠ TESTNET · BINANCE</span>
                    ) : (
                        <span style={{ background: '#f0b90b22', color: "var(--ct-warning, #f0b90b)", border: '1px solid #f0b90b44', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: 1 }} title="Binance MAINNET">● MAINNET · BINANCE</span>
                    )
                ) : exchangeConnecting ? (
                    ...
                    <span style={{ background: '#f5a62322', color: "var(--ct-warning, #f5a623)", border: '1px solid #f5a62344', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>◎ PAPER</span>
                )}
                ...
                {connect.tradingMode === 'LIVE' ? (
                    <button onClick={() => confirmDisconnect('Hyperliquid', connect.disconnectHL)} style={{ background: '#ff3b5c22', color: "var(--ct-negative, #ff3b5c)", border: '1px solid #ff3b5c44', borderRadius: 4, padding: '2px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', letterSpacing: 1 }}>DISCONNECT</button>
                ) : connect.tradingMode === 'LIVE_BINANCE' ? (
                    <button onClick={() => confirmDisconnect('Binance', connect.disconnectBinance)} style={{ background: '#ff3b5c22', color: "var(--ct-negative, #ff3b5c)", border: '1px solid #ff3b5c44', borderRadius: 4, padding: '2px 10px', fontSize: 11, fontWeight: 700, cursor: 'pointer', letterSpacing: 1 }}>DISCONNECT</button>
                ) : (
                    <>
                        <button onClick={() => connect.setShowHlModal(true)} style={{ background: '#00d99222', color: "var(--ct-positive, #00d992)", border: '1px solid #00d99244', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <button onClick={() => connect.setShowBnbModal(true)} style={{ background: '#f0b90b22', color: "var(--ct-warning, #f0b90b)", border: '1px solid #f0b90b44', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                <button className={`nt-sound-btn ${notifEnabled ? 'on' : 'off'}`} onClick={toggleNotif} title={notifEnabled ? 'Masaüstü bildirimlerini kapat' : 'Masaüstü bildirimlerini aç'} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    <IconMonitor />
                    <span style={{ fontSize: 9, color: workspaceTextColor(notifEnabled ? 'var(--accent)' : 'var(--text-3)') }}>{notifEnabled ? 'ON' : 'OFF'}</span>
                </button>
                <button onClick={() => setShowShortcuts(s => !s)} title="Komut Yardımı (? tuşu)" style={{ background: showShortcuts ? 'var(--accent)' : 'transparent', color: workspaceTextColor(showShortcuts ? "var(--ct-ink, #000)" : 'var(--text-3)'), border: '1px solid var(--border-0)', borderRadius: 4, padding: '2px 8px', fontSize: 12, fontWeight: 700, cursor: 'pointer', lineHeight: 1.4 }}>?</button>
