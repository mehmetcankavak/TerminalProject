...
const COINS = [
  ...
  'NEO','ZEC','DASH','STRK','JTO','ONDO','W','APE','AR','NOT',
  'CFX','GMT','MINA','TRB','RSR','VIRTUAL','AERO','ZRO','BERA','PNUT',
  'ETHFI','MOVE','BOME','BRETT','MEW','PENGU','POPCAT','PEOPLE','TURBO',
  'KAITO','GRASS','MOODENG','ME','SPX',
]
      ...
      {EXCHANGES.map(ex => {
        ...
        return (
          <div key={ex.key} style={{
            ...
            border: `1px solid ${urgent ? 'rgba(244,63,94,0.2)' : 'rgba(255,255,255,0.06)'}`,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: '#aaa' }}>{ex.short}</span>
              <span style={{ fontSize: 9, color: '#444', marginLeft: 'auto' }}>{ex.interval}h</span>
            </div>

            {/* Countdown */}
            <div style={{
              fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)',
              color: urgent ? '#f43f5e' : '#fff', marginBottom: 6,
            }}>
              {fmtCountdown(remaining)}
            </div>

            {/* Progress bar */}
            <div style={{ height: 2, background: 'rgba(255,255,255,0.08)', borderRadius: 1, marginBottom: 5 }}>
              <div style={{
                height: '100%', borderRadius: 1, transition: 'width 1s linear',
                width: pct * 100 + '%',
                background: urgent ? '#f43f5e' : '#00d992',
              }} />
            </div>
...
function HistoryChart({ coin }) {
  ...
  if (loading) return (
    <div style={{ padding: '20px 0', textAlign: 'center', color: '#444', fontSize: 20 }}>
      <span style={{ animation: 'm-spin 1s linear infinite', display: 'inline-block' }}>◌</span>
    </div>
  )
  if (!history.length) return (
    <div style={{ padding: '16px 0', textAlign: 'center', color: '#444', fontSize: 12 }}>Veri yok</div>
  )
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <span style={{ fontSize: 11, color: '#555', fontWeight: 600 }}>Binance · Son 90 Dönem</span>
        <span style={{ fontSize: 11, fontWeight: 700, fontFamily: 'var(--mono)', color: annual >= 0 ? '#00d992' : '#f43f5e' }}>
          Yıllık ~{(annual * 100).toFixed(1)}%
        </span>
      </div>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="56" preserveAspectRatio="none" style={{ display: 'block' }}>
          ...
          {rates.map((r, i) => {
            ...
            return <rect key={i} x={x} y={y} width={barW} height={barH}
              fill={r >= 0 ? 'rgba(0,217,146,0.8)' : 'rgba(244,63,94,0.8)'} />
          })}
        </svg>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: '#444', marginTop: 4, fontFamily: 'var(--mono)' }}>
...
function CoinDetail({ coin, data, onBack, isFav, onToggleFav }) {
  ...
  const rates = EXCHANGES.map(ex => ({
    ...ex,
    rate: data[ex.key]?.[coin]?.rate ?? null,
  }))
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100%', color: 'var(--text)', paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', overflow: 'hidden', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {!imgErr && logo
              ...
              : <span style={{ fontSize: 12, fontWeight: 800, color: '#555' }}>{coin.slice(0,3)}</span>
            }
          </div>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800 }}>{coin}</div>
            <div style={{ fontSize: 11, color: '#444' }}>Perpetual Funding Rate</div>
          </div>
        </div>
        <button onClick={() => { haptic('light'); onToggleFav(coin) }}
          style={{ background: 'none', border: 'none', color: isFav ? '#f59e0b' : '#444', fontSize: 22, cursor: 'pointer', padding: 0 }}>
          ★
        </button>
      </div>
      ...
      {avgRate != null && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '16px 20px 8px' }}>
          {[
            { label: 'Ortalama', val: fmtRate(avgRate), color: rateColor(avgRate) },
            { label: 'En Yüksek', val: fmtRate(maxRate), color: rateColor(maxRate) },
            { label: 'En Düşük',  val: fmtRate(minRate), color: rateColor(minRate) },
          ].map(s => (
            <div key={s.label} style={{
              background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)',
              borderRadius: 12, padding: '10px 12px',
            }}>
              <div style={{ fontSize: 10, color: '#555', fontWeight: 600, marginBottom: 4 }}>{s.label}</div>
              <div style={{ fontSize: 14, fontWeight: 800, fontFamily: 'var(--mono)', color: s.color }}>{s.val}</div>
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{ padding: '8px 20px' }}>
        <div style={{ fontSize: 11, color: '#555', fontWeight: 700, letterSpacing: 1, marginBottom: 10 }}>BORSA RATELARI</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {rates.map(ex => (
            <div key={ex.key} style={{
              ...
              <img src={ex.logo} alt={ex.label} width={20} height={20}
                style={{ borderRadius: 4, objectFit: 'cover', flexShrink: 0, marginRight: 10 }}
                onError={e => e.target.style.display = 'none'} />
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#fff' }}>{ex.label}</div>
                <div style={{ fontSize: 10, color: '#444', marginTop: 1 }}>Her {ex.interval}s ödeme</div>
              </div>
              <div style={{
                fontSize: 15, fontWeight: 800, fontFamily: 'var(--mono)',
                color: ex.rate != null ? rateColor(ex.rate) : '#333',
              }}>
                {ex.rate != null ? fmtRate(ex.rate) : '—'}
              </div>
              {/* Countdown */}
              <div style={{ marginLeft: 12, textAlign: 'right', minWidth: 52 }}>
...
function CountdownBadge({ interval }) {
  ...
  return (
    <div style={{ fontSize: 10, fontWeight: 700, fontFamily: 'var(--mono)', color: urgent ? '#f43f5e' : '#555' }}>
      {fmtCountdown(ms)}
    </div>
  )
}
...
function CoinRow({ coin, data, isFav, onSelect, onToggleFav }) {
  ...
  const validRates = rates.filter(r => r.rate != null)
  const avgRate = validRates.length ? validRates.reduce((a, b) => a + b.rate, 0) / validRates.length : null
  // Cross-exchange spread = max rate - min rate. Larger spread = bigger
  // funding arb opportunity (long cheap venue, short expensive venue).
  const spread = validRates.length >= 2
    ? Math.max(...validRates.map(r => r.rate)) - Math.min(...validRates.map(r => r.rate))
    : null
  // Highlight only meaningful spreads — 0.03%+ is worth a trader's attention
  const spreadHot = spread != null && Math.abs(spread) >= 0.0003

  return (
    <div onClick={() => { haptic('light'); onSelect(coin) }}
      style={{ padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', background: 'var(--bg)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        {/* Logo */}
        <div style={{ width: 32, height: 32, borderRadius: '50%', overflow: 'hidden', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          {!imgErr && logo
            ? <img src={logo} alt={coin} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={() => setImgErr(true)} />
            : <span style={{ fontSize: 10, fontWeight: 800, color: '#555' }}>{coin.slice(0,3)}</span>
          }
        </div>

        {/* Symbol */}
        <span style={{ fontSize: 14, fontWeight: 700, fontFamily: 'var(--mono)', flex: 1 }}>{coin}</span>

        {/* Spread badge — only when notable */}
        {spread != null && (
          <span style={{
            fontSize: 9, fontWeight: 800, fontFamily: 'var(--mono)',
            color: spreadHot ? '#fbbf24' : '#666',
            background: spreadHot ? 'rgba(251,191,36,0.10)' : 'transparent',
            padding: spreadHot ? '2px 6px' : '0',
        ...
        {/* Avg rate */}
        {avgRate != null && (
          <span style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: rateColor(avgRate) }}>
            {fmtRate(avgRate)}
          </span>
        )}

        {/* Fav star */}
        <button onClick={e => { e.stopPropagation(); haptic('light'); onToggleFav(coin) }}
          style={{ background: 'none', border: 'none', color: isFav ? '#f59e0b' : '#2a2a2a', fontSize: 16, cursor: 'pointer', padding: '0 0 0 6px', lineHeight: 1 }}>
          ★
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        {rates.map(r => (
          <div key={r.key} style={{
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: '#444', letterSpacing: 0.3 }}>{r.short}</span>
            <span style={{ fontSize: 11, fontWeight: 800, fontFamily: 'var(--mono)', color: r.rate != null ? rateColor(r.rate) : '#2a2a2a' }}>
    ...
    return (
      <div style={{ padding: '12px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        <div style={{ fontSize: 10, color: '#555', fontWeight: 700, letterSpacing: 0.5 }}>
          SENTIMENT · FUNDING · yükleniyor…
        </div>
      </div>
    )
  }
  const tone = sentiment.verdict === 'BULLISH' ? '#00d992'
    ...
    <div style={{ padding: '12px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
      ...
      <div style={{ position: 'relative', height: 8, marginBottom: 8 }}>
        ...
        <div style={{
          ...
          background: 'rgba(255,255,255,0.18)', transform: 'translateX(-50%)',
        }} />
        <div style={{
      ...
      {/* 4-card panel — premium look matching the rest of the app */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
        <div style={{
          background: 'rgba(0,217,146,0.06)', border: '1px solid rgba(0,217,146,0.15)',
          borderRadius: 10, padding: '8px 8px', display: 'flex', flexDirection: 'column', gap: 2,
        }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: '#00d992', letterSpacing: 0.5 }}>OVERSOLD</div>
          <div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: '#fff' }}>{sentiment.oversold_count}</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, fontWeight: 800, color: '#f43f5e', letterSpacing: 0.5 }}>OVERBOUGHT</div>
          <div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: '#fff' }}>{sentiment.overbought_count}</div>
          <div style={{ fontSize: 9, color: '#888', fontFamily: 'var(--mono)' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, fontWeight: 800, color: '#aaa', letterSpacing: 0.5 }}>AVG RATE</div>
          <div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: sentiment.avg_rate >= 0 ? '#f43f5e' : '#00d992' }}>
            {sentiment.avg_rate >= 0 ? '+' : ''}{avgPct}%
          </div>
          <div style={{ fontSize: 9, color: '#888', fontFamily: 'var(--mono)' }}>{sentiment.total_symbols} coin</div>
        </div>
        <div style={{
          background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.15)',
          borderRadius: 10, padding: '8px 8px', display: 'flex', flexDirection: 'column', gap: 2,
        }}>
          <div style={{ fontSize: 9, fontWeight: 800, color: '#fbbf24', letterSpacing: 0.5 }}>ARB OPPS</div>
          <div style={{ fontSize: 13, fontWeight: 800, fontFamily: 'var(--mono)', color: '#fff' }}>{sentiment.arb_count}</div>
          <div style={{ fontSize: 9, color: '#888', fontFamily: 'var(--mono)' }}>
            {sentiment.top_arb[0]
...
export default function FundingRateScreen() {
  ...
  // Backend tracker polls 5 exchanges every 60s and stores latest snapshot.
  // Client just fetches the aggregate — no more 5x cross-origin requests from
  // mobile, less battery drain, and data is fresh even when app was just
  // opened (server has been collecting in the background).
  const fetchAll = useCallback(async () => {
    try {
      const r = await fetch(`${API_BASE}/api/funding/snapshot`)
      if (!r.ok) throw new Error('http_' + r.status)
      const json = await r.json()
      // Server shape: { rates: { BTC: { binance: {rate,...}, bybit: {...} } } }
      // Client expected shape: { binance: { BTC: {rate, ...} }, bybit: { ... } }
      const out = {}
      ...
      for (const [coin, byEx] of Object.entries(json.rates || {})) {
        for (const [exKey, payload] of Object.entries(byEx)) {
          ...
          out[exKey][coin] = { rate: payload.rate, nextFundingTime: payload.next_funding_ms }
        }
      }
      setData(out)
    } catch {
      // Fall back to direct exchange polling if backend unreachable
  ...
  // Funding sentiment — server-computed, refreshed every 30s.
  useEffect(() => {
    let alive = true
    async function pull() {
      try {
  ...
  const sortLabels = {
    default:       'Varsayılan',
    spread_desc:   'Spread: Yüksek → Düşük',
    avg_desc:      'Rate: Yüksek → Düşük',
    avg_asc:       'Rate: Düşük → Yüksek',
    binance_desc:  'Binance: Yüksek → Düşük',
    hl_desc:       'HyperLiq: Yüksek → Düşük',
  }
  ...
  const displayed = useMemo(() => {
    ...
    const avgRate = (coin) => {
      const rs = ratesArr(coin)
      return rs.length ? rs.reduce((a, b) => a + b, 0) / rs.length : null
    }
    const spreadOf = (coin) => {
      const rs = ratesArr(coin)
      return rs.length >= 2 ? Math.max(...rs) - Math.min(...rs) : null
    }
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100%', color: 'var(--text)', paddingBottom: 24 }}>
      ...
      <div style={{ padding: '14px 20px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#444', marginTop: 2 }}>
              {displayed.length} coin · 5 borsa · 30sn güncelleme
            </div>
          </div>

          {/* Favorites toggle */}
          <button
            onClick={() => { haptic('light'); setShowFavs(v => !v) }}
            style={{
              ...
              color: showFavs ? '#f59e0b' : '#666', fontSize: 12, fontWeight: 700,
            }}>
        ...
        {showSort && (
          <div style={{
            ...
            zIndex: 50, boxShadow: '0 10px 40px rgba(0,0,0,0.7)', minWidth: 200,
          }}>
      ...
      {/* Funding sentiment — derived from the DB-backed snapshot */}
      <FundingSentiment sentiment={sentiment} />

      {/* Exchange countdown strip */}
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px 20px', textAlign: 'center', color: '#555', fontSize: 13 }}>
          {showFavs ? 'Favori coin eklenmemiş' : 'Coin bulunamadı'}
        </div>
      ) : (
        displayed.map(coin => (
          <CoinRow
