...
async function main() {
  ...
  try {
    ...
    await page.locator('.mobile-sidebar .ws-sb-item').filter({ hasText: /^Terminal/ }).click()
    ...
    await page.getByRole('button', { name: 'Menu', exact: true }).click()
    await page.locator('.mobile-sidebar .ws-sb-item').filter({ hasText: /^Dashboard/ }).click()
    // Keyboard navigation and account forms remain reachable.
    await page.setViewportSize({ width: 1440, height: 1000 })
    await navigate('dashboard')
    assert.equal(await page.locator('.db-tile').count(), 4, 'Dashboard shows the four coin tiles')
    await page.getByRole('textbox', { name: 'Search watchlist' }).fill('ETH')
    assert.equal(await page.locator('.db-watch tbody tr').count(), 1)
    await page.getByRole('textbox', { name: 'Search watchlist' }).fill('')
    await navigate('smart-money')
    assert.equal(await page.locator('.smx-traders tbody tr').count(), testTraders.length)
    await page.locator('.smx-traders tbody tr').nth(1).click()
    await page.locator('.smx-detail').waitFor()
    await page.screenshot({ path: `${output}/1440-smart-money-detail.png` })
    await navigate('stocks')
    await page.locator('.st-table tbody tr').nth(1).click()
    await page.locator('.st-meta').waitFor()
    ...
    assert.equal(await page.locator('.ws-modal input[type=password]').count(), 3)
    await page.locator('.ws-modal .ws-iconbtn').first().click()
